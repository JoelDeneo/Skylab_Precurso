...
//CUENTRA ATRÁS + EMPIEZAN PREGUNTAS
function countDown() {
...
//COMPRUEBA SI SE HA COMPLETADO, SINO LANZA PREGUNTA
function completed() {
...
function completedStatus() {
	...
	if (right === 27) {
		...
		document.getElementById("result_status").innerHTML = "Has completado el rosco <br> de Pasapalabra!";
	}
}
...
function noMoreTime() {
	document.getElementById("game_interface").style.display = "none";
	document.getElementById("result_box").style.display = "flex";
	document.getElementById("result_status").innerHTML = "Oh, lástima! <br> Se ha terminado el tiempo";
	ranking();
}
...
function ranking() {
	...
	for (let i = 0; i < rankingTable.length; i++) {
		document.getElementById("ranking_box").innerHTML += "Jugador: " + rankingTable[i].Nombre + "	" + "Aciertos: " + rankingTable[i].Aciertos + "	" + rankingTable[i].Completo + "<br>";
	}
}
...
//PENDIENTE
// -Asignar teclas a "responder" y "pasapalabra"s
// -Boton abandono
// -Arreglar tiempo
